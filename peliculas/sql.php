...
class sql extends mysqli{
    ...
    //Funcion donde crea los usuarios en la base de datos
    ...
    //Funcion que nos muestra si estamos logeados o no comparando el email
    ...
    //Funcion que inserta los comentarios en la base de datos
    ...
    //Funcion para sacar los comentarios y los usuarios
    public function imprimir_comentarios($id){
        $sql = "SELECT c.id, c.idUsuario, c.comentario, u.usuario FROM comentarios c INNER JOIN usuarios u ON (c.idUsuario = u.id) WHERE idPelicula =".$id.";";
        $this->default();
        $query = $this->query($sql);
        $this->close();
        $return = Array();
        ...
        while($resultado = $query->fetch_assoc()) {//hacemos que sea una array asociativa
            ...
            $return[] = new join_user($resultado['id'],$resultado['idUsuario'],$resultado['usuario'],$resultado['comentario']);
        }
        return $return;

    }

    //Funcion para insertar la puntuacion
    public function insertar_puntuacion($id_pelicula,$id_usuario, $puntuacion){
        $sql = "INSERT INTO puntuacion (id_peliculas,id_usuario, puntos) VALUES (".$id_pelicula.",".$id_usuario.",".$puntuacion.");";
        $this->default();
        $this->query($sql);
        $this->close();
    ...
    //Funcion que hace un select de la media de los puntos
    public function media_puntuacion($id){
        $sql = "SELECT id, id_peliculas, id_usuario, AVG(puntos) as puntos FROM puntuacion WHERE id_peliculas =".$id.";";
        $this->default();
        $query = $this->query($sql);
        $this->close();
        $return = Array();
        while ($resultado = $query->fetch_assoc()){
            $return[] = new puntos($resultado['id'],$resultado['id_peliculas'],$resultado['id_usuario'], $resultado['puntos']);
        }
        return $return;
    }
